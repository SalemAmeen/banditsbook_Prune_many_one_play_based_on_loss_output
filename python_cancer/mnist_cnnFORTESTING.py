...
for t in range(horizon):
    ...
    for arm_one in chosen_arm:

        get_feature = K.function([model.layers[0].input, K.learning_phase()], model.layers[1].output)
        layer_output = get_feature([X_test, 0])[0] # 0 -> train=false
        outputNeuron = layer_output[arm_one]
        reward = delta + outputNeuron
...
for t in range(MaxofPrune):
    ...
    score, accuracy = model.evaluate(X_deploy, labelsDepy, batch_size=30, verbose=0)
